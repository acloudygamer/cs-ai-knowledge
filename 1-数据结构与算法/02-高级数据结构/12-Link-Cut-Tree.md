...
access 将根到 $x$ 的路径变为 $x$ 的 preferred path（即 $x$ 的左子树链）。Splay(x) 后， $x$ 的左子树正好是 $x$ 到原树根的路径。 的路径变为 $x$ 的 preferred path（即 $x$ 的左子树链）。Splay(x) 后， $x$ 的左子树正好是 $x$ 到原树根的路径。 的 preferred path（即 $x$ 的左子树链）。Splay(x) 后， $x$ 的左子树正好是 $x$ 到原树根的路径。 的左子树链）。Splay(x) 后， $x$ 的左子树正好是 $x$ 到原树根的路径。 的左子树正好是 $x$ 到原树根的路径。 到原树根的路径。

**数学表达**：access 后， $x$ 的左子树中序遍历  $= \text{path}(\text{root}, x)$，且按深度升序排列。 的左子树中序遍历  $= \text{path}(\text{root}, x)$ ，且按深度升序排列。，且按深度升序排列。
...
flip（翻转标记）翻转以 $x$ 为根的整棵辅助树，从而将 $x$ 翻为原树根。这改变了原树的父子关系。 为根的整棵辅助树，从而将 $x$ 翻为原树根。这改变了原树的父子关系。 翻为原树根。这改变了原树的父子关系。
...
此时 $v$ 的左子树包含 $u \to v$ 路径上的所有节点， $v$ 自身的值（即  $\text{val}[v]$）可通过 $\text{push\_up}(v)$ 获得路径聚合结果。 的左子树包含 $u \to v$ 路径上的所有节点， $v$ 自身的值（即  $\text{val}[v]$）可通过 $\text{push\_up}(v)$ 获得路径聚合结果。 路径上的所有节点， $v$ 自身的值（即  $\text{val}[v]$）可通过 $\text{push\_up}(v)$ 获得路径聚合结果。 自身的值（即  $\text{val}[v]$ ）可通过 $\text{push\_up}(v)$ 获得路径聚合结果。）可通过 $\text{push\_up}(v)$ 获得路径聚合结果。 获得路径聚合结果。
...
**preferred path 的核心不变量**：每条实边 $(p, c)$ 中， $c$ 是 $p$ 的 heavy child。任何时刻，一个节点只能有一条实边连向子节点（heavy child），其他子连接均为虚边。 中， $c$ 是 $p$ 的 heavy child。任何时刻，一个节点只能有一条实边连向子节点（heavy child），其他子连接均为虚边。 是 $p$ 的 heavy child。任何时刻，一个节点只能有一条实边连向子节点（heavy child），其他子连接均为虚边。 的 heavy child。任何时刻，一个节点只能有一条实边连向子节点（heavy child），其他子连接均为虚边。

### Splay 的摊还分析

LCT 的摊还复杂度来源于 Splay 的势能分析。设 $n$ 为节点数， $m$ 为操作次数。 为节点数， $m$ 为操作次数。 为操作次数。
...
**关键不变量**：实路径总数不超过 $n$ ，虚边连接数不超过 $n$ 。这保证了 Splay 树不会退化为链。 ，虚边连接数不超过 $n$ 。这保证了 Splay 树不会退化为链。 。这保证了 Splay 树不会退化为链。
...
| 路径操作 | $O(\log^2 n)$ | $O(\log n)$ | | $O(\log n)$ | |
| 子树操作 | $O(\log n)$ | $O(\log n)$ （access 后可操作） | | $O(\log n)$ （access 后可操作） | （access 后可操作） |
| 实现难度 | 低 | 高 |
| 均摊 vs 最坏 | 最坏保证 | 均摊保证 |
