...
核心洞察：用概率替代确定性平衡，以随机性换取实现简洁性。期望 $O(\log n)$ 高度，与红黑树相同的复杂度保证，但代码量少一个数量级。 高度，与红黑树相同的复杂度保证，但代码量少一个数量级。
...
设 Treap 包含 $n$ 个节点，根的 priority 为最大值（概率 $1/n$ ），其余 $n-1$ 个节点递归构成左右子树。 个节点，根的 priority 为最大值（概率 $1/n$ ），其余 $n-1$ 个节点递归构成左右子树。 ），其余 $n-1$ 个节点递归构成左右子树。 个节点递归构成左右子树。

令 $H_n$ 为树高随机变量（期望值）。若根的 rank（优先级排名）为 $i$ ，则左子树 $i-1$ 个节点，右子树 $n-i$ 个节点： 为树高随机变量（期望值）。若根的 rank（优先级排名）为 $i$ ，则左子树 $i-1$ 个节点，右子树 $n-i$ 个节点： ，则左子树 $i-1$ 个节点，右子树 $n-i$ 个节点： 个节点，右子树 $n-i$ 个节点： 个节点：
...
相比 AVL（ $\approx 1.44 \log_2 n$ ）和红黑树（  $\approx 2 \log_2 n$），Treap 的期望高度与 AVL 相当。 ）和红黑树（  $\approx 2 \log_2 n$ ），Treap 的期望高度与 AVL 相当。），Treap 的期望高度与 AVL 相当。
...
**按位置分裂**：将序列从第 $k$ 个位置分裂为 $[1..k]$ 和 $[k+1..n]$ 两部分。 个位置分裂为 $[1..k]$ 和 $[k+1..n]$ 两部分。 和 $[k+1..n]$ 两部分。 两部分。

### 归约终点

Treap 的本质是**随机化的 BST**，随机 priority 替代了红黑树的颜色约束，堆性质（priority 偏序）替代了平衡因子的显式维护。

**与红黑树的关系**：红黑树通过旋转维护颜色约束的平衡，Treap 通过随机 priority 模拟平衡效果。两者都保证 $O(\log n)$ 的期望/最坏树高，但实现复杂度差异显著。 的期望/最坏树高，但实现复杂度差异显著。
...
红黑树通过旋转（单旋/双旋）恢复平衡，每次插入/删除可能触发 $O(\log n)$ 次旋转。Treap 的插入路径下降过程中，**不进行任何旋转**——新节点按 BST 性质插入后，再沿路径回溯做**heapify-up**（类似二叉堆的上浮），这是因为 priority 随机分布后，插入节点成为根的概率极低，沿路径回溯次数的期望是 $O(\log n)$ 。 次旋转。Treap 的插入路径下降过程中，**不进行任何旋转**——新节点按 BST 性质插入后，再沿路径回溯做**heapify-up**（类似二叉堆的上浮），这是因为 priority 随机分布后，插入节点成为根的概率极低，沿路径回溯次数的期望是 $O(\log n)$ 。 。

更精确地说：Treap 的插入等价于"先按 key 下降插入，再按 priority 做一次 bubble-up"。由于 priority 完全随机，插入节点的 priority 大于其所有祖先的概率为 $1/n$ ，因此绝大多数情况下只需局部调整。 ，因此绝大多数情况下只需局部调整。
...
旋转次数期望 $O(\log n)$ ，因为 priority 随机分布，待删节点被选中做大幅旋转的概率极低。 ，因为 priority 随机分布，待删节点被选中做大幅旋转的概率极低。
...
| 最坏情况 | 退化为 $O(n)$ （概率约 $2^{-n}$ ） | 不可能退化 | （概率约 $2^{-n}$ ） | 不可能退化 | ） | 不可能退化 |
...
- **约束**：Treap 不是完全平衡的——存在极低概率（ $n$ 个节点中，随机 priority 的极值排序恰好降序）导致 $O(n)$ 高度，但该概率随 $n$ 指数衰减。 个节点中，随机 priority 的极值排序恰好降序）导致 $O(n)$ 高度，但该概率随 $n$ 指数衰减。 高度，但该概率随 $n$ 指数衰减。 指数衰减。
