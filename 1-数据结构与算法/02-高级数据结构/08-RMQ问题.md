...
设 $k=\lfloor\log_2(r-l+1)\rfloor$， $2^k\le r-l+1<2^{k+1}$。两查询区间 $I_1=[l,l+2^k-1]$、 $I_2=[r-2^k+1,r]$， $I_1\cup I_2=[l,r]$。记 $m=\min_{i\in[l,r]}A[i]$， $m$ 必属 $I_1$ 或 $I_2$ （或两者），故 $m=\min(\min_{I_1}A,\min_{I_2}A)$。

> **洞察（关键约束）**：此论证仅对**幂等操作**成立。sum 重叠会重复计数（结果翻倍）、gcd 重叠会错。故稀疏表框架仅适用于幂等操作：min、max、AND、OR——不适用于 sum/gcd 这类非幂等操作。

### 为什么是 $\lfloor\log_2(r-l+1)\rfloor$

$I_1$ 下界 $l$、 $I_2$ 上界 $r$，两者均长 $2^k$。 $k$ 过大（$2^k>r-l+1$）则 $I_1$ 上界超 $r$； $k$ 过小则两区间间留缝隙。 $\lfloor\log_2(r-l+1)\rfloor$ 是唯一使两区间不越界不留缝的取值。
...
> **洞察**：RMQ 可归约为"区间合并的幂等半环"——任何幂等操作（min/max/AND/OR）都能套同一稀疏表框架。动态场景（数组会改）则线段树更优：稀疏表单点更新要改 $\log n$ 层共 $O(\log n)$ 条记录，区间更新更难。
...
预处理已存所有长 $2^k$ 区间极值。查询按区间长度选 $k$，合并两个可重叠的 $2^k$ 段——两个数组查表 + 一次 min，常数时间。代价是 $O(n\log n)$ 预处理与空间。

### 动态 RMQ 的选择

静态（数组不变）：稀疏表最优。动态（元素会更新）：线段树更适——稀疏表单点更新要改 $\log n$ 层 $O(\log n)$ 条记录，区间更新更难。选择原则：查询数 $Q$ 与更新数 $U$， $Q\gg n\log n/U$ （查询远多于更新）稀疏表仍可选，否则线段树。
