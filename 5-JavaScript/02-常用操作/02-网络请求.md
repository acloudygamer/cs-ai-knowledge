...
XMLHttpRequest 是浏览器早期的异步请求 API，虽然已被 fetch 取代，但在旧代码中仍可见到。
...
fetch 是现代浏览器的原生网络请求 API，基于 Promise，支持流式处理。
...
axios 是最流行的 HTTP 请求库，提供更友好的 API 和更完善的功能。
...
WebSocket 提供双向实时通信通道，适用于聊天、实时数据推送等场景。
...
GraphQL 是 API 查询语言，允许客户端精确请求所需数据。
