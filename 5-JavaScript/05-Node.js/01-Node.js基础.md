...
**事件循环调度模型**：设事件循环每次迭代处理 $n$ 个就绪回调，每个回调的执行时间为 $t_i$ ，则单次迭代耗时 $T = \sum_{i=1}^{n} t_i$ 。若某回调执行时间过长（如 CPU 密集型计算），会阻塞后续所有回调——这是事件循环的核心约束。 个就绪回调，每个回调的执行时间为 $t_i$ ，则单次迭代耗时 $T = \sum_{i=1}^{n} t_i$ 。若某回调执行时间过长（如 CPU 密集型计算），会阻塞后续所有回调——这是事件循环的核心约束。 ，则单次迭代耗时 $T = \sum_{i=1}^{n} t_i$ 。若某回调执行时间过长（如 CPU 密集型计算），会阻塞后续所有回调——这是事件循环的核心约束。 。若某回调执行时间过长（如 CPU 密集型计算），会阻塞后续所有回调——这是事件循环的核心约束。
...
当 $n$ 个任务相互独立时，线程池并行化可将总耗时从 $O(n)$ 降至 $O(\log n)$ 或 $O(1)$ （取决于任务类型和线程池饱和度）。 个任务相互独立时，线程池并行化可将总耗时从 $O(n)$ 降至 $O(\log n)$ 或 $O(1)$ （取决于任务类型和线程池饱和度）。 降至 $O(\log n)$ 或 $O(1)$ （取决于任务类型和线程池饱和度）。 或 $O(1)$ （取决于任务类型和线程池饱和度）。 （取决于任务类型和线程池饱和度）。

**背压的数学约束**：设写入速度 $v_w$ （字节/秒），数据产生速度 $v_d$ （字节/秒），缓冲区大小 $B_{max}$ 。若 $v_w < v_d$ ，缓冲区以速率 $(v_d - v_w)$ 增长： （字节/秒），数据产生速度 $v_d$ （字节/秒），缓冲区大小 $B_{max}$ 。若 $v_w < v_d$ ，缓冲区以速率 $(v_d - v_w)$ 增长： （字节/秒），缓冲区大小 $B_{max}$ 。若 $v_w < v_d$ ，缓冲区以速率 $(v_d - v_w)$ 增长： 。若 $v_w < v_d$ ，缓冲区以速率 $(v_d - v_w)$ 增长： ，缓冲区以速率 $(v_d - v_w)$ 增长： 增长：

$$
\frac{dB}{dt} = v_d - v_w \quad \Rightarrow \quad B(t) = B_0 + (v_d - v_w)t
$$

当 $B(t) \rightarrow B_{max}$ 时，必须暂停生产者以避免溢出。 时，必须暂停生产者以避免溢出。
...
模块解析可视为一个状态机：给定当前路径 $P_{cur}$ 和模块标识 $r$ ，解析结果唯一确定。解析路径构成一棵优先遍历树： 和模块标识 $r$ ，解析结果唯一确定。解析路径构成一棵优先遍历树： ，解析结果唯一确定。解析路径构成一棵优先遍历树：
...
**模块缓存的数学性质**：模块在首次加载后被缓存于 `require.cache`。设缓存键为绝对路径 $p$ ，则同一 `$p$ 在同一进程生命周期内只执行一次。这保证了模块初始化的单次性。 ，则同一 ` $p$ 在同一进程生命周期内只执行一次。这保证了模块初始化的单次性。 在同一进程生命周期内只执行一次。这保证了模块初始化的单次性。
...
进程资源约束可建模为上限函数：
- 文件描述符上限： $FD_{max}$ （通常 1024-65536，受 OS ulimit 限制） （通常 1024-65536，受 OS ulimit 限制）
- 内存上限： $MEM_{max}$ （受 OS 虚拟内存和位数限制） （受 OS 虚拟内存和位数限制）
- 子进程数上限： $CHILD_{max}$ （受 PID 范围和进程表限制） （受 PID 范围和进程表限制）
- V8 堆内存上限： $HEAP_{max}$ （默认约 1.76GB，64 位） （默认约 1.76GB，64 位）
...
**违反约束的后果**：
- 内存泄漏累积超过 $MEM_{max}$ → OS 触发 OOM Killer（Linux）或进程崩溃（Windows） → OS 触发 OOM Killer（Linux）或进程崩溃（Windows）
...
当 $B(t) > B_{high}$ 时暂停生产；当 $B(t) < B_{low}$ 时恢复生产。 时暂停生产；当 $B(t) < B_{low}$ 时恢复生产。 时恢复生产。
