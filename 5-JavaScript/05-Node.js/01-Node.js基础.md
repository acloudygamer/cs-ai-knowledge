...
- Node.js 20+ 中 `Buffer.from()`、`Buffer.alloc()` 和 `Buffer.allocUnsafe()` 的行为保持一致，但推荐使用 `Buffer.alloc()` 以避免未初始化内存暴露
...
crypto 模块本质是对 OpenSSL 的封装，提供摘要、对称加密、非对称加密、密钥派生等能力。Node.js 24+ 中 crypto 模块支持 WebCrypto API 的部分功能，实现更好的跨平台一致性。
