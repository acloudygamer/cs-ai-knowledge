...
其中 $\ominus$ 表示权重键的比较结果（负数/零/正数）。

**Unicode 排序算法的数学基础**：UCA 定义了一个加权键序列 $\{P_1, S_1, T_1, Q_1\}$ 用于每个字符，其中：
- $P_1$ ：主权重（最关键，区分语义）
- $S_1$ ：次权重（区分重音/变体）
- $T_1$ ：三级权重（区分大小写）
- $Q_1$ ：四级权重（其他变体）
...
**复数规则的数学表示**：每种语言的复数规则可表示为一个分段函数 $f: \mathbb{Z} \rightarrow C$ ，其中 $C$ 是该语言的复数类别集合。规则由条件表达式定义（如 "n % 10 == 1 && n % 100 != 11"）。
...
['1', '2', '5', '11', '21'].map(n => `${n}: ${pr.select(n)}`);
