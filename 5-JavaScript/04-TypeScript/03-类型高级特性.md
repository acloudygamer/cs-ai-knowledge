...
类型环境 $\Gamma$ 是从变量名到类型的映射。类型收窄通过控制流分析更新 $\Gamma$：
...
**infer 的模式匹配**：
- `infer U` 仅在 `extends` 条件类型的 true 分支中有效
- 编译器通过模式匹配从 `T` 中提取 `U` 的候选类型
- 支持嵌套 infer：`T extends Promise<infer U> ? U extends string[] ? U[0] : never : T`
...
**合并的优先级**：
- 基础类型与扩展类型：扩展类型的属性覆盖基础类型
- 接口与接口：同键属性取联合类型（若类型不同则形成交叉）
- 类型别名与类型别名：无法合并，会报错
...
**Stage 3 装饰器的数学语义**：
- 装饰器是对值的变换：$Decorator(value, context) \rightarrow newValue$
- 装饰器组合通过函数复合实现：$(D_1 \circ D_2)(value) = D_1(D_2(value))$
...
**逆变的数学证明**：设 $U = A | B$，函数类型 $(k: U) => void$ 的参数类型是 $U$。当 $U = A | B$ 时：
- $(k: A | B) => void$ 等价于 $(k: A) => void \land (k: B) => void$（参数类型的交性）
- 因此联合类型参数被转换为交叉类型
