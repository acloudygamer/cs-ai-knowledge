...
**为什么读布局属性会触发强制同步重排？** 现代浏览器优化为批量写入（将多次 DOM 修改合并为一次重排），但读取时会强制刷新队列——因为读取的值必须是最新计算的布局状态，而批量写入可能尚未应用，所以必须立即计算。这导致读写交替时每次读都触发一次独立的重排，形成布局抖动。
...
**结构化克隆的约束**：克隆过程中，对象被序列化为字节流再反序列化。函数无法序列化（代码无法字符串化后重建闭包），所以函数不能跨线程传递。若尝试传递函数，抛出 `DataCloneError`。

**违反约束的代价**：在 Worker 中执行 CPU 密集任务时，主线程无感知，但 Worker 自身可能进入 Long Task 状态（若任务超过 50ms）。此时 Worker 内部的事件处理同样被阻塞。
...
**违反约束的后果**：
- 若将确定性清理逻辑放入 FinalizationRegistry 回调（如关闭数据库连接），在内存不紧张时 GC 可能延迟执行，导致资源未及时释放
- 若依赖 FinalizationRegistry 进行关键状态同步（如认为回调执行后对象必定被销毁），可能导致竞态条件
