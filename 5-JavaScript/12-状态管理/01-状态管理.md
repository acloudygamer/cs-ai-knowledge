...
其中：
- **状态集** $S$ ：所有合法应用状态的集合 ：所有合法应用状态的集合
- **二元运算** $\cdot: S \times S \to S$ ：动作组合（先后执行两个动作等价于执行组合后的单一动作） ：动作组合（先后执行两个动作等价于执行组合后的单一动作）
- **单位元** $s_0 \in S$ ：初始状态 ：初始状态

约简函数 $r: S \times A \to S$ 满足结合律： 满足结合律：

$$
r(r(s, a_1), a_2) = r(s, a_1 \cdot a_2)
$$

其中 $a_1 \cdot a_2$ 表示先执行 $a_1$ 再执行 $a_2$ 的组合动作。 表示先执行 $a_1$ 再执行 $a_2$ 的组合动作。 再执行 $a_2$ 的组合动作。 的组合动作。
...
其中 $s = (s_1, s_2, \ldots, s_n)$ 是状态树的分裂。每个子 reducer $r_i$ 只负责自己的状态分支 $s_i$ ，子分支的约简互不干扰。 是状态树的分裂。每个子 reducer $r_i$ 只负责自己的状态分支 $s_i$ ，子分支的约简互不干扰。 只负责自己的状态分支 $s_i$ ，子分支的约简互不干扰。 ，子分支的约简互不干扰。
...
`combineReducers` 将根 reducer 函数 $R: S \times A \to S$ 分解为子 reducer 的直积： 分解为子 reducer 的直积：

$$
R(s, a) = \left( r_1(s_1, a), r_2(s_2, a), \ldots, r_n(s_n, a) \right)
$$

其中 $s = (s_1, s_2, \ldots, s_n)$ 是状态树的分裂。 是状态树的分裂。
...
function createSlice({ name, initialState, reducers }) {
  ...
  for (const [key, reducerFn] of Object.entries(reducers)) {
    actionCreators[key] = (payload) => ({ type: ` ${name}/$ {key}`, payload });{key}`, payload });
  }
...
- **节点** $V$ ：Observable 属性、Computed 属性、Reaction（autorun/reaction） ：Observable 属性、Computed 属性、Reaction（autorun/reaction）
- **有向边** $E \subseteq V \times V$ ：从 Observable 属性指向依赖它的 Computed/Reaction ：从 Observable 属性指向依赖它的 Computed/Reaction

当 Observable $o$ 被赋值时，MobX 执行： 被赋值时，MobX 执行：
...
**环检测的数学约束**：DAG 必须满足 $\forall v \in V,$ 不存在从 $v$ 到 $v$ 的路径。设检测算法为 $O(|V| + |E|)$ ，每次赋值触发环检测。 不存在从 $v$ 到 $v$ 的路径。设检测算法为 $O(|V| + |E|)$ ，每次赋值触发环检测。 到 $v$ 的路径。设检测算法为 $O(|V| + |E|)$ ，每次赋值触发环检测。 的路径。设检测算法为 $O(|V| + |E|)$ ，每次赋值触发环检测。 ，每次赋值触发环检测。
...
设 Computed 求值代价为 $C_{eval}$ ，缓存有效期为 $T_{cache}$ （无限期，直到依赖变化）。则： ，缓存有效期为 $T_{cache}$ （无限期，直到依赖变化）。则： （无限期，直到依赖变化）。则：
- 未变化时：访问代价 $O(1)$ 
- 变化后首次访问：代价 $C_{eval}$ 
...
设事务内进行 $n$ 次赋值。事务机制将通知次数从 $O(n)$ 降为 $O(1)$ ： 次赋值。事务机制将通知次数从 $O(n)$ 降为 $O(1)$ ： 降为 $O(1)$ ： ：
...
设一个 Store 有 $N$ 个 state 字段和 $M$ 个 computed.getter，组件只使用其中 $k$ 个字段。当任意字段变化时，Vue 的响应式系统通过 Proxy get trap 建立依赖图，**只有依赖该字段的 computed 和组件**才会被标记为脏（dirty）。 个 state 字段和 $M$ 个 computed.getter，组件只使用其中 $k$ 个字段。当任意字段变化时，Vue 的响应式系统通过 Proxy get trap 建立依赖图，**只有依赖该字段的 computed 和组件**才会被标记为脏（dirty）。 个 computed.getter，组件只使用其中 $k$ 个字段。当任意字段变化时，Vue 的响应式系统通过 Proxy get trap 建立依赖图，**只有依赖该字段的 computed 和组件**才会被标记为脏（dirty）。 个字段。当任意字段变化时，Vue 的响应式系统通过 Proxy get trap 建立依赖图，**只有依赖该字段的 computed 和组件**才会被标记为脏（dirty）。
...
**数学描述**：设 store 对象为 $S$ ，属性 $k$ 的响应式引用 $ref_k$ 满足： ，属性 $k$ 的响应式引用 $ref_k$ 满足： 的响应式引用 $ref_k$ 满足： 满足：
