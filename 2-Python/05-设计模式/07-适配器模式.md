...
适配器模式将一个类的接口转换成客户期望的另一个接口，使原本不兼容的类可以一起工作。适用于集成第三方库、兼容不同接口的场景。

## 类适配器

类适配器通过多重继承实现，在 `request()` 中调用 `specific_request()` 并转换。
...
适配器模式实际应用：集成不同支付网关（Stripe、PayPal），统一 `charge()` 接口。
...
适配器模式用于文件格式转换（XML、CSV），统一 `process()` 接口。
