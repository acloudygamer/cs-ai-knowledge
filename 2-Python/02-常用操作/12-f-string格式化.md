...
**约束**：这意味着 `%Y` 和 `>` 不能同时用于 datetime——strftime 格式规格符与通用对齐规格符是**互斥的**，不能混合使用。
...
**展开过程**：
1. 词法阶段：`f"{x=}"` → token 序列含 `FormattedValue(value=Name('x'), conversion=-1)`
2. 字节码生成：检测 `conversion=-1`，生成追加变量名文本的指令
3. 运行时输出：`x=42`
