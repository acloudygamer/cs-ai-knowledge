...
> **版本注解**：本章所述特性为 Python 3.14 前沿版本特性，部分特性可能不稳定或处于实验阶段。稳定版本请参阅 Python 3.12 章节。

## 定义

Python 3.14 是前沿版本，包含多项重大语言特性、标准库扩展和性能改进。模板字符串 t-strings 提供安全的延迟渲染；类型提示惰性求值（PEP 649）消除模块加载时的类型解析开销；子解释器支持实现真正的多核并行；自由线程模式（PEP 703，Python 3.13 引入）继续完善打破 GIL 瓶颈。
...
其中 $\text{convert}$ 根据 conversion 标志（`!r`、`!s`、`!a`）应用 `repr`/`str`/`ascii` 转换。 根据 conversion 标志（`!r`、`!s`、`!a`）应用 `repr`/`str`/`ascii` 转换。
...
**惰性求值的开销分析**：
- 立即求值：导入时完成所有类型解析， $O(n)$ 复杂度（ $n$ 为类型表达式深度） 复杂度（ $n$ 为类型表达式深度） 为类型表达式深度）
- 惰性求值：首次访问时解析，后续访问 $O(1)$ （缓存命中） （缓存命中）
...
**子解释器的并行度**：若在 $n$ 个子解释器中运行 $n$ 个 CPU 密集型任务，则有效并行度为 $n$ （无 GIL 竞争）。但子解释器间通信需要序列化，存在额外开销。 个子解释器中运行 $n$ 个 CPU 密集型任务，则有效并行度为 $n$ （无 GIL 竞争）。但子解释器间通信需要序列化，存在额外开销。 个 CPU 密集型任务，则有效并行度为 $n$ （无 GIL 竞争）。但子解释器间通信需要序列化，存在额外开销。 （无 GIL 竞争）。但子解释器间通信需要序列化，存在额外开销。
...
```bash
python3.13t -X gil=0 my_script.py  # 自由线程构建（3.13）
```
...
优化后， $f$ 的栈帧被 $g$ 的栈帧替代，等价于： 的栈帧被 $g$ 的栈帧替代，等价于： 的栈帧替代，等价于：
...
| t-string | 不支持 | PEP 750 模板字符串（provisional） |
| except* 语法 | `except* (A, B)` | `except* A, B`（括号可选） |
| UUID | v1/v3/v4/v5 | v6/v7/v8（新增） |
| 尾调用 | 不支持 | 实验性提案，尚未合并 |
| 自由线程 | 不支持 | Python 3.13 引入，3.14 完善 |
