...
一句话断言本质。不是描述，是断言。避免空洞的"是一种...的技术"表述，直指本质。
...
ASCII 图或分步列表，标记数据从入口到出口的每一次形态变换和所有权转移。与数学模型并列，两者都需存在，不可取舍。

> **ASCII 图**：用 `<pre>` 标签包裹，防止空格被 Markdown 渲染器忽略导致对齐错乱。

## 对比参照

独立块，置于数据流之后、机制之前。若存在易混淆概念（如协程/线程/进程），用三列对比表格：内存模型、调度权、数据交换代价。若无易混淆概念，直接省略此块。
