...
当 $B \gg s$ 时，缓冲区方案显著减少 syscall 次数。
...
| 方案 | 优点 | 缺点 |
|------|------|------|
| 继承（每种组合一个类） | 简单 | 类爆炸：$n$ 种数据源 $\times$ $m$ 种装饰 = $n \times m$ 个类 |
...
设装饰器链深度为 $D$，每次 I/O 的方法调用开销为 $C_{method}$（约 10-50ns）。对于 $N$ 次字节读取：

$$T_{decorator}(N) = N \cdot (D \cdot C_{method} + C_{syscall})$$

当 $D=3$ 且 $N=1,000,000$ 时，方法调用开销累计约 30-150ms，相比 I/O 时间可忽略。但对于高频低延迟场景（如网络代理），这一开销不可忽视。
