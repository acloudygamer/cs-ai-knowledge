...
设待序列化对象图 $G = (V, E)$ ，其中 $V$ 为节点集合（对象字段）， $E$ 为引用边集合（对象引用）。序列化时间复杂度： ，其中 $V$ 为节点集合（对象字段）， $E$ 为引用边集合（对象引用）。序列化时间复杂度： 为节点集合（对象字段）， $E$ 为引用边集合（对象引用）。序列化时间复杂度： 为引用边集合（对象引用）。序列化时间复杂度：
...
其中 $s$ 为单个节点平均字节开销（约 40-80 字节，含类型标记和父子指针）。 为单个节点平均字节开销（约 40-80 字节，含类型标记和父子指针）。

Jackson 的 `XmlFactory` 流式解析器仅维护当前路径栈，内存占用：

 $M_{stream} = O(d \cdot s)$ 

其中 $d$ 为最大嵌套深度，通常 $d \ll |V|$ 。 为最大嵌套深度，通常 $d \ll |V|$ 。 。

**对比**：对于 1000 个节点的 JSON，树模式需分配 ~80KB 节点对象，流式模式仅需 ~4KB（假设最大深度 10）。

### 循环引用压缩率

Jackson 的 `@JsonIdentityInfo` 为每个对象分配唯一标识符 $\text{oid}$ 。设对象图中唯一对象数为 $|U|$ ，出现次数为 $f_i$ ，总引用数为 $R = \sum_{i=1}^{|U|} f_i$ 。压缩后输出边数： 。设对象图中唯一对象数为 $|U|$ ，出现次数为 $f_i$ ，总引用数为 $R = \sum_{i=1}^{|U|} f_i$ 。压缩后输出边数： ，出现次数为 $f_i$ ，总引用数为 $R = \sum_{i=1}^{|U|} f_i$ 。压缩后输出边数： ，总引用数为 $R = \sum_{i=1}^{|U|} f_i$ 。压缩后输出边数： 。压缩后输出边数：

 $R' = |U| + (R - |U|) = R$ 

压缩收益在 $f_i > 1$ 时显著：当同一对象被引用多次（如父引用子、孙引用爷形成环），边数不变但对象内容只输出一次。 时显著：当同一对象被引用多次（如父引用子、孙引用爷形成环），边数不变但对象内容只输出一次。
...
`ObjectMapper` 在首次遇到类型 $T$ 时，通过 `SerializerProvider` 查找或构建 `JsonSerializer<T>`。查找路径： 时，通过 `SerializerProvider` 查找或构建 `JsonSerializer<T>`。查找路径：
...
每 `read()`/`write()` 操作从链首到链尾线性扫描，最坏时间复杂度 $O(n)$ ，其中 $n$ 为注册的 `TypeAdapterFactory` 数量（通常 < 20）。 ，其中 $n$ 为注册的 `TypeAdapterFactory` 数量（通常 < 20）。 为注册的 `TypeAdapterFactory` 数量（通常 < 20）。
...
设对象图中存在环 $C = (v_1 \to v_2 \to \cdots \to v_k \to v_1)$ 。Jackson 的处理算法： 。Jackson 的处理算法：

1. **首次访问**：输出完整对象内容，并记录 $\text{oid}(v_i)$ 
2. **后续访问**：输出 `{"$ref": "oid_of_vi"}`
3. **回边检测**：通过 IdentityHashMap 跟踪已访问对象

这将原始有环图转化为**生成树 + 回边标记**的 DAG。设环中节点数为 $k$ ，输出边数： ，输出边数：

 $|E'| = |E| - k + 1$ 

因为每个环节省了 $k-1$ 条边的内容输出（替换为一条回边引用）。 条边的内容输出（替换为一条回边引用）。
