...
设池大小为 $P$ ，活跃连接数为 $A$ ，空闲连接数为 $I$ ，则： ，活跃连接数为 $A$ ，空闲连接数为 $I$ ，则： ，空闲连接数为 $I$ ，则： ，则：

 $A + I = P$ 

最大并发请求数受 $P$ 限制。若 $A = P$ ，新请求等待或失败。 限制。若 $A = P$ ，新请求等待或失败。 ，新请求等待或失败。
...
设事务 $T_1, T_2$ 并发执行， $T_1$ 读取数据  $x$， $T_2$ 修改 $x$ 并提交， $T_1$ 再次读取  $x$： 并发执行， $T_1$ 读取数据  $x$， $T_2$ 修改 $x$ 并提交， $T_1$ 再次读取  $x$： 读取数据  $x$ ， $T_2$ 修改 $x$ 并提交， $T_1$ 再次读取  $x$：， $T_2$ 修改 $x$ 并提交， $T_1$ 再次读取  $x$： 修改 $x$ 并提交， $T_1$ 再次读取  $x$： 并提交， $T_1$ 再次读取  $x$： 再次读取  $x$ ：：
...
设参数为 $p_1, p_2, ..., p_n$ ，执行计划为 $Plan$ 。参数绑定后： ，执行计划为 $Plan$ 。参数绑定后： 。参数绑定后：
...
设泄漏检测阈值为 $T_{leak}$ ，连接借出时间为 $t_{checkout}$ ，则泄漏判定： ，连接借出时间为 $t_{checkout}$ ，则泄漏判定： ，则泄漏判定：
