...
Record 的核心语义是**积类型**（product type）： $R(c_1: T_1, c_2: T_2, ..., c_n: T_n)$ 的实例是各组件的笛卡尔积。 的实例是各组件的笛卡尔积。

### 数学模型

**组件访问器**：设 Record 类型 $R$ 有组件 $(c_1: T_1, c_2: T_2, \dots, c_n: T_n)$ ，访问器语义： 有组件 $(c_1: T_1, c_2: T_2, \dots, c_n: T_n)$ ，访问器语义： ，访问器语义：
...
javac 编译
    ...
    ├── 生成 Point $R$ 1.class（内部补充类，存储组件元数据）1.class（内部补充类，存储组件元数据）
...
从集合论视角，sealed 接口 $S$ 的 `permits $C_1, C_2, ..., C_n 定义了一个**有限类型族**：$S$ 的所有实例必须是 $C_i$ 的某种子类型。 的 `permits $C_1, C_2, ..., C_n 定义了一个**有限类型族**：$ S$ 的所有实例必须是 $C_i$ 的某种子类型。S $ 的所有实例必须是 $ C_i$ 的某种子类型。C_i$ 的某种子类型。

### 数学模型

设密封族 $S$ 的直接子类集合为 $P = \{C_1, C_2, \dots, C_n\}$ ，每个子类 $C_i$ 的密封状态： 的直接子类集合为 $P = \{C_1, C_2, \dots, C_n\}$ ，每个子类 $C_i$ 的密封状态： ，每个子类 $C_i$ 的密封状态： 的密封状态：

 $\text{sealedStatus}(C_i) \in \{\text{sealed}, \text{non-sealed}, \text{final}\}$ 

**穷尽性要求**：任何覆盖 $S$ 的 `switch` 表达式必须满足： 的 `switch` 表达式必须满足：
...
**类型测试模式** $P :: T$ 对值 $v$ 的匹配语义： 对值 $v$ 的匹配语义： 的匹配语义：
...
设虚拟线程集合 $VT = \{v_1, v_2, \dots, v_m\}$ ，载体线程集合 $PT = \{p_1, p_2, \dots, p_n\}$ ，其中 $m \gg n$ 。 ，载体线程集合 $PT = \{p_1, p_2, \dots, p_n\}$ ，其中 $m \gg n$ 。 ，其中 $m \gg n$ 。 。
...
**关键不变量**：同一时刻每个载体线程 $p_j$ 最多承载一个虚拟线程执行。 $p_j$ 阻塞时，其承载的 $v_i$ 被移出到等待队列。 最多承载一个虚拟线程执行。 $p_j$ 阻塞时，其承载的 $v_i$ 被移出到等待队列。 阻塞时，其承载的 $v_i$ 被移出到等待队列。 被移出到等待队列。

**内存模型**：

| 线程类型 | 栈内存 | 总内存复杂度 |
|----------|--------|--------------|
| 传统线程 | $N \times 1\,\text{MB}$ | $O(N)$ | | $O(N)$ | |
| 虚拟线程 | $N \times \text{按需增长（~256KB-2MB）}$ | $O(N)$ ，但常数小 | | $O(N)$ ，但常数小 | ，但常数小 |
...
|------|-------------|-------------|
| 虚拟线程开销 | 每 VT 独立副本 $O(N_{VT})$ | 共享数据 $O(1)$ | | 共享数据 $O(1)$ | |
