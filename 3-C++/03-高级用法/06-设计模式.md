...
常见问题（如对象创建、对象组合、行为组织）有通用解决方案，设计模式将这些方案标准化并给出实现指导。

创建型模式：单例、工厂方法、抽象工厂、建造者。结构型模式：适配器、装饰器、组合、代理。行为型模式：观察者、策略、命令、模板方法、迭代器。现代 C++ 用智能指针、lambda、`std::function` 简化实现。
...
int main() {
    ...
    auto group = std::make_unique<CompositeGraphic>();
