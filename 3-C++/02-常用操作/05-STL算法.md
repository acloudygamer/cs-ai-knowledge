...
**比较排序的复杂度下界**：任何基于比较的排序算法在最坏情况下至少需要 $\Omega(n \log n)$ 次比较操作。 次比较操作。

**信息论证明**： $n!$ 种可能的排列需要 $\log_2(n!)$ 位信息来唯一区分。使用比较作为二元决策，每次比较最多产生 1 bit 信息，因此至少需要 $\lceil \log_2(n!) \rceil$ 次比较。 种可能的排列需要 $\log_2(n!)$ 位信息来唯一区分。使用比较作为二元决策，每次比较最多产生 1 bit 信息，因此至少需要 $\lceil \log_2(n!) \rceil$ 次比较。 位信息来唯一区分。使用比较作为二元决策，每次比较最多产生 1 bit 信息，因此至少需要 $\lceil \log_2(n!) \rceil$ 次比较。 次比较。
...
Introsort 是三种算法的动态混合，根据递归深度 $d$ 自适应切换： 自适应切换：
...
`vec.erase(new_end, vec.end())` 删除 $[new\_end, end)$ 区间的元素，调用析构函数并触发移动。删除 $k$ 个元素的代价：析构 $k$ 个对象 + 移动 $n-k$ 个对象到新位置。 区间的元素，调用析构函数并触发移动。删除 $k$ 个元素的代价：析构 $k$ 个对象 + 移动 $n-k$ 个对象到新位置。 个元素的代价：析构 $k$ 个对象 + 移动 $n-k$ 个对象到新位置。 个对象 + 移动 $n-k$ 个对象到新位置。 个对象到新位置。

### 归并排序的递归复杂度

设 $T(n)$ 为归并排序的运行时间： 为归并排序的运行时间：
...
对于 $n$ 个元素、 $p$ 个处理器的并行执行，理想加速比受 **Amdahl 定律** 约束： 个元素、 $p$ 个处理器的并行执行，理想加速比受 **Amdahl 定律** 约束： 个处理器的并行执行，理想加速比受 **Amdahl 定律** 约束：

$$
S(p) = \frac{1}{(1 - f) + \frac{f}{p}}
$$

其中 $f$ 是可并行化比例。对于 `par` 算法（工作窃取调度）： 是可并行化比例。对于 `par` 算法（工作窃取调度）：
- 可并行部分： $T_{par}(n, p) = \frac{O(n)}{p} + O(\log n)$ （ $\log n$ 为同步开销） （ $\log n$ 为同步开销） 为同步开销）
- 不可并行部分（串行）： $O(\log n)$ 

`par_unseq` 进一步允许**向量化**（单指令多数据），理论 throughput 提升至 $p \times v$ （ $v$ 为 SIMD 宽度）。 （ $v$ 为 SIMD 宽度）。 为 SIMD 宽度）。
...
`binary_search`：返回"是否存在"（布尔值）。等价于 `std::binary_search` 检查 $[first, last)$ 中是否有等于 value 的元素。 中是否有等于 value 的元素。
...
`std::nth_element(first, nth, last)` 保证：经过调用后，`*nth` 是排序后第 $k$ 个元素（ $k$ 为 nth 在 [first, last) 中的排名），且所有小于 `*nth` 的元素在它之前，所有大于它的元素在它之后。 个元素（ $k$ 为 nth 在 [first, last) 中的排名），且所有小于 `*nth` 的元素在它之前，所有大于它的元素在它之后。 为 nth 在 [first, last) 中的排名），且所有小于 `*nth` 的元素在它之前，所有大于它的元素在它之后。

**时间复杂度**：线性 $O(n)$ （快速选择算法），而非全排序的 $O(n \log n)$ 。 （快速选择算法），而非全排序的 $O(n \log n)$ 。 。
