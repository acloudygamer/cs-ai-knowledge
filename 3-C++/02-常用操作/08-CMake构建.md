...
| 性质 | 描述 | 在 CMake 中的意义 |
|------|------|------------------|
| 无环性 | $\nexists (A, B) \in E^+: (A, A) \in E$ | 禁止循环依赖 |
| 传递性 | $(A, B) \in E \land (B, C) \in E \Rightarrow (A, C) \in E$ | 间接依赖成立 |
...
| 表达式 | 语义 |
|--------|------|
| $\$<CONFIG:Debug>:X$ | Debug 配置下为 X，否则为空 |
| $\$<NOT:$<CONFIG:Debug>>:X$ | 非 Debug 下为 X |
| $\$<AND:$<CONFIG:Debug>,$<CONFIG:Release>>:X$ | 同时满足时为 X（无意义，永空） |
| $\$<OR:$<CONFIG:Debug>,$<CONFIG:Release>>:X$ | 任一满足时为 X |
