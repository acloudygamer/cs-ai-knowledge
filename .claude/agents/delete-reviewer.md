...
## Step 1: 深度扫描 (Deep Scan)
- **禁止“望文生义”（反偷懒指令）**：绝对禁止仅通过 `ls` 或 `tree` 查看文件名就得出结论！你必须使用文件读取命令（如 `cat`、`head` 或你的专属阅读工具）抽查或全量读取核心 `.md` 文件的真实内部文本。
- **锚定过时特征**：在读取文本时，主动寻找低于 `<stable>` 版本的特征词（例如在 Java 目录寻找 "Java 8", "Date API"；在 JS 目录寻找 "var", "回调地狱"）。
- **圈定清单**：基于你真实读取到的文本证据，明确圈定出“待删除列表”和“待合并清单”。
