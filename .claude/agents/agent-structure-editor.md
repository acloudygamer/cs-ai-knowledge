...
- 编号连续性检查
- 缺失文件检测
- **重复编号检测**：检查同一父目录下是否有多个同编号子目录（如两个 `04-`）

### 重复编号修复
发现重复编号时，按以下规则重命名：
- 同一级别目录编号不能重复
- 保持内容相关性：同主题的目录保持相邻编号
- 重命名后更新所有内部文件的相对路径引用（如有）
- 修复后验证目录结构
...
2. 统计文件数量、行数
3. 识别薄文件和结构问题
4. **重点检查重复编号**：扫描所有子目录，检测是否有多个同编号目录（如 `04-测试/` 和 `04-设计模式/` 同时存在）
5. 生成审查报告

### 循环后修复
1. 根据审查报告修复问题
2. **修复重复编号**：当发现同一父目录下有多个同编号子目录时，重命名冲突目录使编号连续唯一
3. 创建缺失文件
4. 修复内容错误
5. 生成修复报告
