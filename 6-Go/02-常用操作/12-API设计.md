...
RESTful API 是"资源即 URL、方法即动名词"的无状态 HTTP 接口，约束是**客户端-服务器**分离、**无状态**交互、**统一接口**——这些约束使得缓存和横向扩展成为可能。
...
**幂等性形式化**：操作 $f$ 是幂等的，当且仅当： 是幂等的，当且仅当：
 $\forall x: f(f(x)) = f(x)$ 
...
**偏移分页的问题**：当 $N_{total}$ 在分页过程中增加时： 在分页过程中增加时：
- 假设总页数 $N_{pages} = 10$ ，用户在第 5 页时新增 2 条记录 ，用户在第 5 页时新增 2 条记录
