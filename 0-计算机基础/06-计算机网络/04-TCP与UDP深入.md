...
**为什么四次挥手是必要的**：被动关闭方收到FIN后先回ACK，此时可能还有数据要发送，所以FIN由被动方单独发送。这导致FIN和ACK分开，产生四次挥手。主动关闭方的FIN意味着"我不再发送数据"，但仍能接收数据；被动关闭方的FIN意味着"我也完成了发送"。

**为什么2MSL等待**：确保最后的ACK到达对方，且旧连接的重复报文在网络中完全消失。MSL（Maximum Segment Lifetime）是报文在网络中最长存活时间估计，通常为60秒或120秒。2MSL确保往返方向的报文都消失。
...
**流量控制 vs 拥塞控制**：
- 流量控制：接收方告知发送方自己的缓冲区剩余空间，防止发送方淹没接收方
- 拥塞控制：发送方根据网络反馈（丢包、超时）调整发送速率，防止网络过载

两者共同约束发送窗口：$\text{SentBytesUnacked} + \text{SentNotYetAcked} \leq \min(cwnd, rwnd)$

**拥塞控制的阶段转换**：慢启动从小窗口开始，指数增长到ssthresh后进入拥塞避免（线性增长）。超时意味着严重丢包，ssthresh折半，cwnd重置为1 MSS。3个重复ACK意味着轻度拥塞（部分丢包），执行快速重传和快速恢复（cwnd折半而非重置）。
...
**QUIC的创新**：QUIC在用户态实现可靠传输、多路复用、0-RTT握手、连接迁移（通过Connection ID）。用户态意味着协议可以独立演进，不受TCP内核实现约束，且可以快速部署。
