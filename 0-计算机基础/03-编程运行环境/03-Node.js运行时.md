...
其中 $\lambda$ 是请求到达率， $\mu$ 是每个线程的服务率， $c$ 是线程池大小。当 $\rho \to 1$ 时，队列延迟趋向无穷大。 是请求到达率， $\mu$ 是每个线程的服务率， $c$ 是线程池大小。当 $\rho \to 1$ 时，队列延迟趋向无穷大。 是每个线程的服务率， $c$ 是线程池大小。当 $\rho \to 1$ 时，队列延迟趋向无穷大。 是线程池大小。当 $\rho \to 1$ 时，队列延迟趋向无穷大。 时，队列延迟趋向无穷大。
...
同步模型中线程在 I/O 等待时 $T_\text{I/O等待}$ 计入分母但 $T_\text{计算}$ 无法增加，CPU 利用率极低。事件驱动模型通过切换到其他任务使 $T_\text{计算}$ 在等待期间增加，从而提升利用率。 计入分母但 $T_\text{计算}$ 无法增加，CPU 利用率极低。事件驱动模型通过切换到其他任务使 $T_\text{计算}$ 在等待期间增加，从而提升利用率。 无法增加，CPU 利用率极低。事件驱动模型通过切换到其他任务使 $T_\text{计算}$ 在等待期间增加，从而提升利用率。 在等待期间增加，从而提升利用率。
...
| epoll_wait | $O(1)$ （返回就绪的文件描述符数量） | （返回就绪的文件描述符数量） |
...
状态转移由 I/O 事件触发，转移代价仅为 $O(1)$ 的函数调用。 的函数调用。
