...
**异构计算**指使用不同类型处理器（CPU + GPU/FPGA/专用芯片）协同完成计算任务。GPU 擅长数据并行的高吞吐量计算，与 CPU 的灵活单线程控制互补。
...
**关键连接**：
- GPU → **并行计算**：GPU 拥有数千个小核心，适合 SIMT 执行
- CUDA/OpenCL → **GPU 编程**：主流 GPU 编程框架
- CPU-GPU 互联 → **PCIe/NVLink**：数据传输带宽影响性能
- 异构调度 → **任务划分**：将合适任务分配给合适处理器
