...
终端行规程是一个 **Moore 机**，状态空间 $Q$ 由以下变量共同决定： 由以下变量共同决定：
...
其中：
- $q_{\text{canon}} \in \{\text{Collecting},\ \text{Complete}\}$ ：canonical 模式的行缓冲状态 ：canonical 模式的行缓冲状态
- $q_{\text{echo}} \in \{0, 1\}$ ：ECHO 模式标志 ：ECHO 模式标志
- $q_{\text{echo\_pend}} \in \{\text{None},\ \text{Char},\ \text{Erase}\}$ ：待回显字符类型 ：待回显字符类型
- $B \in \Sigma^{\leq 255}$ ：当前积累的字节缓冲区 ：当前积累的字节缓冲区
- $t_{\text{last}}$ ：最后接收字符的时刻（用于 $TIME$ 超时判定） ：最后接收字符的时刻（用于 $TIME$ 超时判定） 超时判定）
...
当 $TIME = 0$ （默认值）时，超时交付被禁用，行只有在遇到 EOL/EOF 时才交付。 （默认值）时，超时交付被禁用，行只有在遇到 EOL/EOF 时才交付。
...
**归约终点**：行规程的状态机可归约为 **字节变换函数** $f: \Sigma^{*} \times \Sigma \rightarrow \Sigma^{*} \times \{\text{Deliver}, \text{Collect}, \text{Signal}\}$ ，其输出取决于 ICANON/ECHO/ISIG/ICRNL 等标志位的配置。 ，其输出取决于 ICANON/ECHO/ISIG/ICRNL 等标志位的配置。
...
ECHO 模式下的回显不是即时生效的，而是经过了一个两级缓冲过程：

$$
\text{输入字符} c \xrightarrow{q_{\text{echo}}=1} \text{回显队列} E_{\text{queue}} \xrightarrow{\text{VTIME/VMIN 超时或下一字符}} \text{Master write}
$$

**时序约束**：
- ECHO 产生的字节必须写入 PTY Master 端，而非直接显示
- 若 ECHO 和输入字符的写入速度不匹配，可能导致**回显乱序**（输入字符先于 ECHO 显示）
- 在 Raw 模式下，ECHO 被禁用，每个字符的"回显"由应用程序自行完成
...
**安全约束**（现代 Linux 内核）：
$$
\text{TIOCSTI} \in \text{CAP\_SYS\_ADMIN} \lor \text{TIOCSTI\_OWNER} = \text{current\_process}
$$
普通用户进程无法使用 TIOCSTI 注入字节，这防止了通过其他进程的标准输入进行输入劫持攻击。
...
\text{SIGWINCH} & \text{winsize changed} \land \text{foreground} \\
...
键盘扫描码
    │
    ▼
ring buffer [内核所有，无用户空间可见性]
    │
    ▼
行规程 [内核代理，执行星号注入]
    │
    ▼
PTY Slave fd [进程可见的文件描述符]
    │
    ▼
Shell 进程内存 [用户空间]
```
...
**所有权转移点分析**：
- 键盘→ring buffer：IRQ 处理器在内核态完成，无锁（单生产者）
- ring buffer→行规程：消费者模型，行规程作为内核线程持有
- 行规程→PTY Slave：经由 `read()` 系统调用，数据从内核复制到用户空间
- PTY Master→终端模拟器：经由 `write()` 系统调用
...
**TCP vs PTY EOF 的本质区别**：
- TCP FIN：优雅关闭，发送方已无数据发送，但接收方可能还有未读数据
- PTY Master close：等价于 TCP RST，所有未读数据被丢弃，Slave 端无任何通知
...
**Resize during read 的竞争条件**：
- 若进程在 read() 阻塞期间收到 SIGWINCH，read() 不会被打断
- 进程下次 read() 调用会看到新的 winsize
- vim 等全屏程序通常会注册 SIGWINCH 处理器，主动重新绘制
...
| 属性 | Canonical (Cooked) | CBREAK | Raw |
|------|-------------------|--------|-----|
| ICANON | 开启 | 关闭 | 关闭 |
| ECHO | 开启 | 关闭 | 关闭 |
| ISIG | 开启 | 开启 | 关闭 |
| VMIN | 被忽略 | 1 | 0 |
| VTIME | 被忽略 | 0 | 0 |
| read() 返回 | 整行 | 至少 1 字节 | 任意可用字节 |
| Ctrl+C 信号 | SIGINT | SIGINT | 无（作为字节传递） |

### 对比参照

| 属性 | 虚拟终端 (tty1-F6) | 串口终端 (ttyS) | 伪终端 (pts/N) |
|------|-------------------|-----------------|----------------|
| 物理意义 | 内核多路复用显示 | RS-232/UART | socket-tty 适配 |
| 切换方式 | Alt+F1-F6 | N/A | N/A |
| 数据到达速率 | 屏幕刷新率 | 波特率 | 网络延迟 |
| 缓冲区溢出 | 不可能 | ring buffer 覆盖 | socket 背压 |
| 典型使用者 | 本地登录 | 串口设备 | SSH/xterm |
...
// 查询当前行规程
int ldisc;
ioctl(STDIN_FILENO, TIOCGETD, &ldisc);
```
