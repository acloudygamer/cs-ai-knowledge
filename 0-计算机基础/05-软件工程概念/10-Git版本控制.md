...
| 合并类型 | 时间复杂度 | 空间复杂度 | 说明 |
|----------|------------|------------|------|
| Fast-forward | $O(1)$ | $O(1)$ | 指针移动，无实际合并 | | $O(1)$ | 指针移动，无实际合并 | | 指针移动，无实际合并 |
| 3-way merge | $O(n)$ | $O(n)$ | 比较三个提交 | | $O(n)$ | 比较三个提交 | | 比较三个提交 |
| 递归 3-way merge | $O(n)$ | $O(n)$ | 处理分叉历史 | | $O(n)$ | 处理分叉历史 | | 处理分叉历史 |

其中 $n$ 是需比较的文件数。Fast-forward 发生于合并分支与当前分支无分叉时，直接移动指针即可。 是需比较的文件数。Fast-forward 发生于合并分支与当前分支无分叉时，直接移动指针即可。
...
对于 $k = 10^9$ 个对象（大量），碰撞概率约为 $10^{-31}$ 。 个对象（大量），碰撞概率约为 $10^{-31}$ 。 。
...
**DAG 的数学性质**：设 $C$ 是提交集合， $parent: C \to C^k$ 是 parent 指针函数。Git 的历史是 $parent$ 函数生成的偏序关系。 是提交集合， $parent: C \to C^k$ 是 parent 指针函数。Git 的历史是 $parent$ 函数生成的偏序关系。 是 parent 指针函数。Git 的历史是 $parent$ 函数生成的偏序关系。 函数生成的偏序关系。
