...
**归约内核**：CI/CD 的本质是**软件交付的流水线自动化**。传统手动部署的每个步骤（构建、测试、部署）被编排为自动执行的管道，human-in-the-loop 被最小化，交付速度大幅提升。
...
理想情况下，$T_{pipeline} \approx \max(T_i)$（最慢阶段决定总时间）。实际中，$W_j$ 来源于：
...
部署频率与平均恢复时间（MTTR）强相关（DevOps Research and Assessment 数据）：

$$
MTTR \propto \frac{1}{deployment\_frequency}
$$

高部署频率迫使团队建设快速恢复能力（自动化回滚、特性开关），从而进一步缩短 MTTR，形成正反馈循环。
...
违反幂等性的后果是**环境漂移（Environment Drift）**：不同次部署产生不同的环境状态，排查问题困难。

### 快速失败的质量门禁意义

快速失败（Fail Fast）的核心洞察是：**缺陷发现越晚，修复成本越高**。研究数据（IBM Systems Sciences Institute）表明：
...
违反此约束的后果是**部署不可追溯**：无法确定生产环境实际运行的是什么代码。

### 部署策略的可用性保障

| 策略 | 停机时间 | 回滚速度 | 资源成本 | 可用性风险 |
|------|----------|----------|----------|------------|
| 蓝绿部署 | 零 | 秒级 | 双倍 | 需要 DB 迁移兼容 |
| 金丝雀发布 | 零 | 分钟级 | 增量 | 负载均衡复杂度 |
| 滚动更新 | 零 | 分钟级 | 最小 | 版本共存期 |

蓝绿部署通过维护两套环境（蓝/绿），切换流量实现零停机；金丝雀发布逐步将流量切换到新版本；滚动更新是 Kubernetes 默认策略，逐步替换旧版本 Pod。
...
只要任一依赖变化，根哈希就变化，触发重新构建。这与 Git 的内容寻址存储同构。
